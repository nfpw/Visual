local Library = {Toggle = true,FirstTab = nil,TabCount = 0,ColorTable = {},ActiveNotifications = {}}
...
function MakeDraggable(ClickObject, Object)
	local Dragging = nil
	local DragInput = nil
	local DragStart = nil
...
function Library:CreateWindow(Config, Parent)
	local WindowInit = {}
	local Folder = loadstring(game:HttpGetAsync("https://raw.githubusercontent.com/nfpw/Visual/refs/heads/main/Library/Folder.lua", true))();
	...
	function WindowInit:CreateTab(Name)
		...
		function TabInit:CreateSection(Name)
			...
			function SectionInit:CreateMultiDropdown(Name, OptionTable, Callback, InitialValues)
				local MultiDropdownInit = {}
				local MultiDropdown = Folder.Dropdown:Clone()
				MultiDropdown.Name = Name .. " MD"
				MultiDropdown.Parent = Section.Container
				MultiDropdown.Title.Text = Name
				MultiDropdown.Title.Size = UDim2.new(1, 0, 0, MultiDropdown.Title.TextBounds.Y + 5)
				MultiDropdown.Container.Position = UDim2.new(0, 0, 0, MultiDropdown.Title.TextBounds.Y + 5)
				MultiDropdown.Size = UDim2.new(1, -10, 0, MultiDropdown.Title.TextBounds.Y + 25)
				local SelectedOptions = {}
				if InitialValues and type(InitialValues) == "table" then
				...
				function UpdateText()
					...
					if #SelectedArray == 0 then
						...
						SelectedText = table.concat(SelectedArray, ", ")
						if MultiDropdown.Container.Value.TextBounds.X > MultiDropdown.Container.AbsoluteSize.X - 10 then
							local maxLength = 20
							if string.len(SelectedText) > maxLength then
								SelectedText = string.sub(SelectedText, 1, maxLength) .. "..."
							end
						end
					end
					MultiDropdown.Container.Value.Text = SelectedText
				end
				local DropdownToggle = false
				MultiDropdown.MouseButton1Click:Connect(function()
					DropdownToggle = not DropdownToggle
					if DropdownToggle then
						MultiDropdown.Size = UDim2.new(1, -10, 0, MultiDropdown.Container.Holder.Container.ListLayout.AbsoluteContentSize.Y + MultiDropdown.Title.TextBounds.Y + 30)
						MultiDropdown.Container.Holder.Visible = true
					else
						MultiDropdown.Size = UDim2.new(1, -10, 0, MultiDropdown.Title.TextBounds.Y + 25)
						MultiDropdown.Container.Holder.Visible = false
					end
				end)
				function UpdateOptionVisual(Option, Selected)
					if Selected then
						Option.BackgroundTransparency = 0.7
						Option.Title.TextColor3 = Color3.fromRGB(255, 255, 255)
					else
						Option.BackgroundTransparency = 1
				...
				for _, OptionName in pairs(OptionTable) do
					local Option = Folder.Option:Clone()
					Option.Name = OptionName
					Option.Parent = MultiDropdown.Container.Holder.Container
					Option.Title.Text = OptionName
					Option.BackgroundColor3 = Config.Color
					Option.Size = UDim2.new(1, 0, 0, Option.Title.TextBounds.Y + 5)
					MultiDropdown.Container.Holder.Size = UDim2.new(1, -5, 0, MultiDropdown.Container.Holder.Container.ListLayout.AbsoluteContentSize.Y)
					table.insert(Library.ColorTable, Option)
					...
					Option.MouseButton1Up:Connect(function()
						...
						UpdateText()
						local SelectedArray = {}
						for selected, isSelected in pairs(SelectedOptions) do
							if isSelected then
								table.insert(SelectedArray, selected)
							end
						end
						...
						if type(ReturnValue) == "table" then
							...
							for _, value in pairs(ReturnValue) do
								SelectedOptions[value] = true
							end
							for _, Child in pairs(MultiDropdown.Container.Holder.Container:GetChildren()) do
				...
				function MultiDropdownInit:AddToolTip(Name)
					if tostring(Name):gsub(" ", "") ~= "" then
				...
				function MultiDropdownInit:GetSelectedOptions()
					local Selected = {}
					for option, isSelected in pairs(SelectedOptions) do
						if isSelected then
				...
				function MultiDropdownInit:SetOption(Name, Selected)
					for _, Option in pairs(MultiDropdown.Container.Holder.Container:GetChildren()) do
						if Option:IsA("TextButton") and Option.Name == Name then
							SelectedOptions[Name] = Selected
							UpdateOptionVisual(Option, Selected)
						end
					end
					UpdateText()
					local SelectedArray = {}
				...
				function MultiDropdownInit:RemoveOption(Name)
					for _, Option in pairs(MultiDropdown.Container.Holder.Container:GetChildren()) do
						if Option:IsA("TextButton") and Option.Name == Name then
							Option:Destroy()
				...
				function MultiDropdownInit:ClearOptions()
					for _, Option in pairs(MultiDropdown.Container.Holder.Container:GetChildren()) do
						if Option:IsA("TextButton") then
							Option:Destroy()
						end
				...
				function MultiDropdownInit:SelectAll(Value)
					Value = Value ~= false
					for _, Option in pairs(MultiDropdown.Container.Holder.Container:GetChildren()) do
						if Option:IsA("TextButton") then
							SelectedOptions[Option.Name] = Value
							UpdateOptionVisual(Option, Value)
						end
					end
					UpdateText()
					local SelectedArray = {}
				...
				if InitialValues and type(InitialValues) == "table" then
					for _, Option in pairs(MultiDropdown.Container.Holder.Container:GetChildren()) do
						if Option:IsA("TextButton") then
							UpdateOptionVisual(Option, SelectedOptions[Option.Name])
						end
					end
					UpdateText()
				else
					MultiDropdown.Container.Value.Text = "None"
				end
				return MultiDropdownInit
			end
